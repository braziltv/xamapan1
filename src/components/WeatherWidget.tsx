...
export function WeatherWidget({ currentTime, formatTime }: WeatherWidgetProps) {
    ...
    return (
      <div className="flex items-center gap-3 ml-2">
        <div className="flex items-baseline whitespace-nowrap">
          ...
          <span className="text-base font-mono font-bold text-yellow-400 animate-pulse">
            :{formatTime(currentTime, 'ss')}
          </span>
        </div>
        <div className="text-center">
          <p className="text-[10px] font-mono font-bold text-yellow-400 leading-tight whitespace-nowrap">
            {formatTime(currentTime, "EEE").toUpperCase()}
          </p>
          <p className="text-[10px] font-mono font-semibold text-cyan-400 leading-tight whitespace-nowrap">
  ...
  return (
    <div className="bg-red-800 backdrop-blur-md rounded-xl px-4 py-3 shadow-lg overflow-hidden">
      <div className="flex items-center gap-4">
        {/* Clock first - moved to left */}
        <ClockSection />
        
        <div className="w-px h-10 bg-red-600/50" />
        
        {/* Current weather with city */}
        <div className="flex items-center gap-3">
          <div className="flex flex-col items-center text-white text-sm font-poppins font-semibold drop-shadow-[0_0_6px_rgba(255,255,255,0.5)] leading-tight text-center min-w-[110px]">
            <span className="text-xs">Previsão do tempo</span>
            <span className="flex items-center gap-0.5 animate-pulse text-yellow-400 text-xs"><MapPin className="w-3 h-3 animate-bounce" />{displayCity}-MG</span>
          </div>
          {getWeatherIcon(weather.current.description, 'lg')}
          <div className="flex items-baseline gap-1.5 transition-all duration-500">
            <span className={`text-[9px] font-bold ${showMaxTemp ? 'text-orange-300' : 'text-cyan-300'}`}>
              {showMaxTemp ? 'MAX' : 'MIN'}
            </span>
          ...
          <Droplets className="w-5 h-5 text-cyan-300 ml-1" />
          <span className="text-red-100 text-xs">{weather.current.humidity}%</span>
        </div>
        ...
        {weather.forecast?.slice(0, 2).map((day, index) => {
          ...
          return (
            <div 
              key={index} 
              className="bg-red-700 rounded-lg px-3 py-1.5 flex items-center gap-2"
            >
